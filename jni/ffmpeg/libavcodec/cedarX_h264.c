...
av_cold int decode_init(AVCodecContext *avctx){
	...
	memset(&cedarContext->VideoInfo, 0, sizeof(VideoStreamInfo));
	memset(&cedarContext->VideoConf, 0, sizeof(VConfig));
	...
	cedarContext->VideoInfo.pCodecSpecificData = NULL;
	
	cedarContext->VideoConf.eOutputPixelFormat  = PIXEL_FORMAT_YUV_PLANER_420;//PIXEL_FORMAT_YV12;
	ret = InitializeVideoDecoder(cedarContext->pVideoDec, &cedarContext->VideoInfo, &cedarContext->VideoConf);
